...
module.exports = function(RED) {
    function tinkerForgeTemperatureIR(n) {
        ...
        node.ipcon.on(Tinkerforge.IPConnection.CALLBACK_CONNECTED, function(connectReason) {
            ...
            node.t = new Tinkerforge.BrickletTemperatureIR(node.sensor, node.ipcon);

            node.interval = setInterval(function() {
                    if (node.t) {

                        node.t.getObjectTemperature(function(temp) {
                                var ot = {
                                    topic: node.topic || 'objecTemperature',
                                    payload: temp / 10.0
                                };

                                node.t.getAmbientTemperature(function(temp2) {
                                    var at = {
                                        topic: node.topic || 'ambientTemperature',
                                        payload: temp2 / 10.0
                                    };
