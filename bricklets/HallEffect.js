...
module.exports = function(RED) {
	function tinkerForgeHallEffect(n) {
        ...
        function(connectReason) {
            ...
            node.t = new Tinkerforge.BrickletHallEffect(node.sensor, node.ipcon);

            node.interval = setInterval(function(){
                if (node.t) {
                    node.t.getValue(function(hall) {

                        node.send({
                            topic: node.topic || 'HallEffect',
                            payload: hall
                        });
                    },
                    function(err) {
    ...
    RED.nodes.registerType('TinkerForge HallEffect', tinkerForgeHallEffect);
};
