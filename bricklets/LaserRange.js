...
module.exports = function(RED) {
	function tinkerForgeLaserRange(n) {
        ...
        function(connectReason) {
            node.t = new Tinkerforge.BrickletLaserRangeFinder(node.sensor, node.ipcon);
			node.t.enableLaser();
			setTimeout(function(){ //timeout to allow laser start
				node.send({
                            topic: "laserEnabled"
						  }
						 );
    ...
    RED.nodes.registerType('TinkerForge LaserRange', tinkerForgeLaserRange);
};
