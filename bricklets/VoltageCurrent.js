...
module.exports = function(RED) {
    function tinkerForgeVoltageCurrent(n) {
        ...
        node.ipcon.on(Tinkerforge.IPConnection.CALLBACK_CONNECTED, function(connectReason) {
            ...
            node.t = new Tinkerforge.BrickletVoltageCurrent(node.sensor, node.ipcon);

            node.interval = setInterval(function() {
                    if (node.t) {

                        node.t.getCurrent(function(current) {
                                var c = {
                                    topic: node.topic || 'current',
                                    payload: current
                                };

                                node.t.getVoltage(function(voltage) {
                                    var v = {
                                        topic: node.topic || 'voltage',
                                        payload: voltage
                                    };

                                    node.t.getPower(function(power) {
                                        var p = {
                                            topic: node.topic || 'power',
                                            payload: power
                                        };
    ...
    RED.nodes.registerType('TinkerForge VoltageCurrent', tinkerForgeVoltageCurrent);
};
