...
module.exports = function(RED) {
	function tinkerForgeDistanceIR(n) {
        ...
        function(connectReason) {
            node.t = new Tinkerforge.BrickletDistanceIR(node.sensor, node.ipcon);

            node.interval = setInterval(function(){
                if (node.t) {
                    node.t.getDistance(function(distance) {
                        node.send({
                            topic: node.topic || 'distanceIR',
                            payload: distance
                        });
                    },
                    function(err) {
    ...
    RED.nodes.registerType('TinkerForge DistanceIR', tinkerForgeDistanceIR);
};
